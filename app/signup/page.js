...
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${process.env.API_BASE_URL || "https://api2.pixelprimp.com"}/api/clients/signup`, {
        method: "POST",
      ...
      if (data.success) {
        setToastState({ open: true, type: "success", message: data.message || "Account created successfully!" });
        ...
        // Redirect to homepage after short delay for toast
        setTimeout(() => {
          router.push("/");
        }, 800);
      } else {
        setToastState({ open: true, type: "error", message: data.message || "Signup failed." });
      }
    } catch (err) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-200/60 via-purple-200/60 to-pink-100/60 dark:from-gray-900 dark:to-gray-800 px-2">
      <form
        ...
        autoComplete="off"
      >
        <h2 className="text-3xl font-extrabold text-center mb-2 text-gray-800 dark:text-white tracking-tight">Create your account</h2>
        <p className="text-center text-gray-500 dark:text-gray-300 mb-6 text-base">Sign up to get started with PixelPrimp</p>
        <div className="space-y-4">
          <div>
            <label className="block text-gray-700 dark:text-gray-200 font-semibold mb-1" htmlFor="name">Name</label>
            <input
              type="text"
              name="name"
              id="name"
          ...
          <div>
            <label className="block text-gray-700 dark:text-gray-200 font-semibold mb-1" htmlFor="email">Email</label>
            <input
              type="email"
              name="email"
              id="email"
          ...
          <div>
            <label className="block text-gray-700 dark:text-gray-200 font-semibold mb-1" htmlFor="phone">Phone</label>
            <input
              type="text"
              name="phone"
              id="phone"
          ...
          <div className="relative">
            <label className="block text-gray-700 dark:text-gray-200 font-semibold mb-1" htmlFor="confirmPassword">Confirm Password</label>
            <input
